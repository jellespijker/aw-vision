...
- Be specific and evidence-based. Name the exact artifacts visible on screen: file names, directory paths, function/class names, git branch names, ticket IDs (e.g. PRJ-2026-042), URLs, document titles, terminal commands, spreadsheet tab names, chat participants.
...
5. Classification reasoning ('project_reasoning'): Reason step by step IN THIS FIELD before deciding on a project:
   a. List the strongest pieces of evidence (identifiers, paths, ticket prefixes, repository names, user-provided context).
   b. Compare that evidence against each project in the catalog below; state which candidates match, which are ruled out, and why.
...
7. Technical tags ('tags'): Generate 3 to 7 highly relevant, technical tags. Prioritize reusing these existing database tags: [{existing_tags}]
8. Synthesis ('description'): One ultra-dense, technical "Caveman-style" summary. Speak in fragments, use semicolons, omit filler words (the, a, is, was, were, to, of, for). Every word must carry maximum information, and every concrete identifier from the evidence should survive into it.
...
{user_context_block}- Active Window: {active_window_description}
...
1. project_reasoning: Reason step by step BEFORE deciding:
   a. List the strongest pieces of evidence (identifiers, file paths, ticket prefixes, repository names, the user-provided context note when present).
   b. Compare the evidence against each catalog project; state which candidates match, which are ruled out, and why.
...
2. project_number: Based ONLY on the reasoning above, classify this activity into ONE catalog project. Be conservative: if there is no direct, explicit link between the active screen contents and a project's description/entailment, output "None". Do NOT match on inactive sidebar chats, adjacent tab names, browser bookmarks, or external company profiles.
3. tags: 3 to 7 highly relevant, technical tags/keywords for this task. Prioritize reusing these existing database tags for consistency: {existing_tags}
4. description: An ultra-dense, highly precise "Caveman-style" work summary. Omit filler words (the, a, is, was, were, to, of, for); use dense technical fragments separated by semicolons/periods. Every concrete identifier from the evidence (file names, functions, URLs, tickets) should survive into it — never replace them with generic activity words.
   Example: "Dev aw-vision UI. Refactored GalleryTab.tsx list component; unique elements via exact CSS tokens; PR #22 review."
...
PROMPT_DEFS: List[Dict[str, Any]] = [
    ...
    },
    {
        ...
        "placeholders": ["image_layout_note", "user_context_block", "mcp_context_block", "skills_block"],
