...
* **User-Steerable Analysis**: Every prompt in the pipeline is a user-editable template (**Settings → Prompts**, `prompts.py`) rendered with safe single-brace placeholder substitution. Users can attach **MCP servers** (`mcp_manager.py`) and **Claude Skills** (`skills.py`) to individual prompt slots (`local_vision`, `local_synthesis`, `gemini_combined`, `agent`) to inject external tool context or expert instructions. Skills are uploaded through the UI or auto-discovered from a local skills directory (`config.toml → customization.skills_dir`); disk-discovered skills are read-only in the UI (content lives on disk, assignments/enabled live in the DB). Each snapshot also carries an optional free-text **user context note** (`user_context`) that is treated as authoritative evidence during (re)processing.
